...
function(t, e, n) {
    ...
    function r() {
        if ("over" != y && "pass" != y) {
            var t = P.d || "normal";
            for (z = (t && I[t] || I.e)(Date.now() - k), c.save().fillStyle("#fff").fillRect(0, 0, h, g).restore(), c.B().arc(m, d, v, 0, M, !1).fill(), t = 0; t < q.length; t++) o(q[t] + z);
            ...
            if ("shooting" == y) {
                ...
                T ? (e = w * D(T.a), e = w * A(T.a) + n.sqrt(4 * p * p - e * e), a(t, e), y = "over", c.save().font("bold 18px sans-serif").textAlign("right").fillStyle("#f00").fillText("失败了", h - 10, g - 45).fillText("一秒后重新开始", h - 10, g - 20).restore(), stop(), setTimeout(l, 1e3)) : 1 > e ? a(t, u) : (q.push(i(t - z)), o(t), y = "idle", j || (y = "pass", ("jfcz-data", '{"aa":' + (F + 1) + "}")

, c.save().font("bold 18px sans-serif").textAlign("right").fillStyle("#060").fillText("完美过关", h - 10, g - 45).fillText("一秒后下一关", h - 10, g - 20).restore(), setTimeout(l, 1e3)))
            }
