...
if (bdS.indexOf("本学期成绩") >= 0) { //检测获取数据是否成功
    ...
    $("[name='qb_004']").remove();
    $("[name='qb_005']").remove();
} else if(bdS.indexOf('两学期') >= 0 ) {
    var ts = $('b').length; //总的学期数+总表头(1)
    /*console.log('学期'+ts);*/
    cSem_default = $('b').eq(0).text().replace('学年', '').replace('(两学期)','').replace('3(三学期)', '三');
    tr_start = 0;
    for ( j = 0; j <= ts - 1; j ++) {
        tr_length = $('.displayTag').eq(j).find('tr').length - 1;
        console.log(tr_length);
        for (i = tr_start; i <= tr_start + tr_length - 1; i++) {
            cN.push(clean($('.odd,.even').eq(i).find('td').eq(2).text(), 1));
            cE.push($('.odd,.even').eq(i).find('td').eq(3).text());
            cW.push(clean($('.odd,.even').eq(i).find('td').eq(4).text(), 2));
            cP.push(clean($('.odd,.even').eq(i).find('td').eq(5).text(), 1));
            ...
            cSem.push($('b').eq(j).text().replace('学年', '').replace('(两学期)','').replace('3(三学期)', '三'));
        }
        tr_start = tr_start + tr_length;
    }
    var scores = [];
    cSemS = Array.from(new Set(cSem));
/*    for ( i = 0; i <= cSemS.length; i ++) {
        scores.push([]);
    }*/
    for( i in cSemS ) {
        scores[i] = []
        for ( j in cR ) {
            if (cSem[j] == cSemS[i]) {
                scores[i].push(Number(cR[j]));
            }
        }

        ranges.push([ cSemS[i], Math.min.apply(null, scores[i]), Math.max.apply(null,scores[i])]);
        var sum = eval(scores[i].join("+"));
        averages.push( [ cSemS[i], ~~(sum/scores[i].length*100)/100]);

    }
    console.log(ranges,averages);
} else {
    for (i = 0; i <= tn - 1; i++) {
        cN.push(clean($('.odd,.even').eq(i).find('td').eq(2).text(), 1));
        cE.push($('.odd,.even').eq(i).find('td').eq(3).text());
...
if (bdS.indexOf("本学期成绩") >= 0) {
    ...
    $(".card-table tbody").append('<tr border="0"><th scope="col" style="text-align:left; padding-left:2em;">' + cSem_default + '</th><th scope="col" style="text-align:left; padding-left:2em;"></th></tr>');
    for ( i = 0; i <= tn -1; i++ ) {
        if (cSem[i] != cSem_default) {
            $(".card-table tbody").append('<tr border="0"><th scope="col" style="text-align:left; padding-left:2em;">' + cSem[i] + '</th><th scope="col" style="text-align:left; padding-left:2em;"></th></tr>');
            cSem_default = cSem[i];
        } else {
